...
export const verifyEmail = async (tenantName, token) => {
    try {
      const response = await fetch(`https://${tenantName}.api.fastrasuite.com/email-verify/?token=${token}`, {
        method: 'GET',
        headers: {
  ...
  export const resendVerificationEmail = async (tenantName, token) => {
    try {
      const response = await fetch(`https://${tenantName}.api.fastrasuite.com/resend-verification-email/${token}/`, {
